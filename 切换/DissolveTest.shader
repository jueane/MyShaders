			...
			fixed4 frag(v2f i) : SV_Target{
				...
				finalcolor = lerp(rgb, rgbBg, t);
				 ...
				 if(t>0&&t<5.2){
					...
					if(y/x>0.31+t&&y/x<0.32+t){
					
							finalcolor = float3(_SinTime.x,1-_SinTime.y,_SinTime.z);


					}
				
					if(modf(y,x)>0.30+t&&modf(y,x)<0.32+t){
							finalcolor = float3(t,0,0);

					}


					//if(_SinTime.x<0.5 && _SinTime.x>-0.255 ){
